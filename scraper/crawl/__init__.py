...
class PdfEntry:
    ...
    doc_id: str
    url: str
    size_hint: str = ""     # e.g. "PDF:827KB" from link text
...
def _parse_size_mb(hint: str) -> float | None:
    """Parse '827KB', '1.09MB', '8.27MB' into MB."""
...
def crawl(
    ...
    fnames = [f.name for f in fields(PdfEntry)]
    fnames.append("local_filename")
    ...
    with Progress(
        ...
        for e, filename, dest in to_download:
            progress.update(task, description=filename[:50])
            ...
            # Stream to temp file then rename (avoids partial writes).
    ...
    console.print(f"\n[green]Downloaded {ok}[/green] PDFs to {PDFS_DIR}")
    if errors:
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        metavar="SEC",
        help=f"Seconds to wait between HTTP requests (default: {DEFAULT_DELAY}).",
    )
    parser.add_argument(
        "--max-mb",
        type=float,
